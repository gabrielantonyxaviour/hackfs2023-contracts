...
interface ITheRiotProtocol {
    function isRiotOrganisation(address _address) external view returns (bool);
}
...
contract DealClient {
    ...
    ITheRiotProtocol public riotProtocol;

    constructor(ITheRiotProtocol _riotProtocol) {
        riotProtocol = _riotProtocol;
        owner = msg.sender;
    }
    ...
    modifier onlyRiotOrganisation() {
        require(riotProtocol.isRiotOrganisation(msg.sender), "Unauthorized");
        _;
    }
    ...
    function makeDealProposal(DealRequest calldata deal) public onlyRiotOrganisation returns (bytes32) {
        require(msg.sender == owner);
